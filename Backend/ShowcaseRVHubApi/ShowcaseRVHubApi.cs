...
namespace ShowcaseRVHubApi
{
    /// <summary>
    /// The FabricRuntime creates an instance of this class for each service type instance.
    /// </summary>
    internal sealed class ShowcaseRVHubApi : StatelessService
    {
        public ShowcaseRVHubApi(StatelessServiceContext context)
            : base(context)
        { }
            ...
            return new ServiceInstanceListener[]
            {
                new ServiceInstanceListener(serviceContext =>
                    new KestrelCommunicationListener(serviceContext, "ServiceEndpoint", (url, listener) =>
                    {
                        ...
                        string? password = builder.Configuration["Kestrel:Certificates:Development:Password"];
                        string? certPath = builder.Configuration["Kestrel:Certificates:Development:Path"];
                                    ...
                                    .UseKestrel(opt =>
                                    {
                                        ...
                                        opt.ListenAnyIP(port, listenOptions =>
                                        {
                                            if (!string.IsNullOrEmpty(certPath) || !string.IsNullOrEmpty(password))
                                            {
                                                ...
                                                Console.WriteLine($"Certificate path or passord is null or empty\tPath: {certPath}\tPassword: {password}");
                                        });
                                    })
